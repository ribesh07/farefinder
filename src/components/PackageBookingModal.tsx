...
export default function PackageBookingModal({ isOpen, onClose, pkg }: PackageBookingModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const validated = packageLeadSchema.parse({
        packageId: pkg.id,
        ...formData,
  ...
  const generateWhatsAppLink = () => {
    const message = `
Hello FareFinderUK! I'm interested in this holiday package:
        ...
        <div className="p-6">
          {isSuccess ? (
            <div className="text-center py-8">
              ...
              <h3 className="text-2xl font-bold mb-2">Package Request Submitted!</h3>
            ...
            <>
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-xl font-bold">Holiday Package</h2>
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    {pkg.title} - {pkg.destination} - £{pkg.startingPrice}
                  </p>
                </div>
