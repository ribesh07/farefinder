...
export default function DealBookingModal({ isOpen, onClose, deal }: DealBookingModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const validated = dealLeadSchema.parse({
        dealId: deal.id,
        ...formData,
        travelers: Number(formData.travelers),
      });

      const response = await fetch('/api/leads/deal', {
        method: 'POST',
  ...
  const generateWhatsAppLink = () => {
    const message = `
Hello FareFinderUK! I'm interested in this deal:

Deal Details:
- Title: ${deal.title}
- Type: ${deal.type}
- New Price: £${deal.newPrice}
${deal.oldPrice ? `- Old Price: £${deal.oldPrice}` : ''}
        ...
        <div className="p-6">
          {isSuccess ? (
            <div className="text-center py-8">
              ...
              <h3 className="text-2xl font-bold mb-2">Deal Request Submitted!</h3>
