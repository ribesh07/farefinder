...
export default function EditFlightForm({ id }: { id: string }) {
  ...
  useEffect(() => {
    async function loadFlight() {
      const result = await adminFetch<FlightRecord>(`/flights/${id}`);
      if (result.ok) {
  ...
  async function onSubmit(data: Record<string, unknown>) {
    setIsLoading(true);
    setError(null);
    const result = await adminFetch(`/flights/${id}`, {
      method: "PUT",
      body: JSON.stringify({
        ...data,
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div><Label>From Airport</Label><Input {...register("fromAirport", { required: true })} /></div>
              <div><Label>To Airport</Label><Input {...register("toAirport", { required: true })} /></div>
            </div>
            <ImageUrlField
              label="Logo URL"
              name="logo"
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-1">
              <div><Label>Duration</Label><Input {...register("duration", { required: true })} /></div>
            </div>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              <div><Label>Stops</Label><Input type="number" {...register("stops", { required: true })} /></div>
              <div>
                <Label>Cabin Class</Label>
                <Select value={watch("cabinClass")} onValueChange={(value) => setValue("cabinClass", value)}>
            ...
            <div className="flex gap-4">
              ...
              <Button type="button" variant="ghost" onClick={() => router.push("/admin/flights")}>Cancel</Button>
            </div>
          </form>
        </CardContent>
      </Card>
